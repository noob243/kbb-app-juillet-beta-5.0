...
function App() {
    ...
    useEffect(() => {
        ...
        const unsub = onSnapshot(colRef, (snapshot) => {
            ...
            console.error("Correspondances app-wide subscription error:", err);
        });
        return () => unsub();
    }, [isDbConnected]);
    ...
    const handleDismissAlarm = async () => {
        ...
        try {
            ...
            await dbUpdateDoc<Task>('tasks', id, cleanTask);
        } catch (err) {
    ...
    const handleUpdateEvent = async (updatedEvent: Event) => {
        ...
        try {
            ...
            await dbUpdateDoc<Event>('events', id, cleanEvent);
    ...
    const handleUpdateTaskStatus = async (id: number, status: 'Effectué' | 'Non effectué' | 'Effectué à moitié') => {
        ...
        try {
            await dbUpdateDoc<Task>('tasks', id, { status });
    ...
    const handleUpdateClient = async (updated: Client) => {
        ...
        try {
            ...
            await dbUpdateDoc<Client>('clients', id, properties);
    ...
    const handleUpdateCase = async (updated: Case) => {
        ...
        try {
            ...
            await dbUpdateDoc<Case>('cases', id, clean);
    ...
    const handleUpdateAvocat = async (updated: Avocat) => {
        ...
        try {
            ...
            await dbUpdateDoc<Avocat>('avocats', id, clean);
    ...
    const handleUpdatePersonnel = async (updated: Personnel) => {
        ...
        try {
            const { id, ...properties } = updated;
            await dbUpdateDoc<Personnel>('personnels', id, properties);
    ...
    const handleUpdateInvoice = async (updated: Invoice) => {
        ...
        try {
            const { id, ...properties } = updated;
            await dbUpdateDoc<Invoice>('invoices', id, properties);
    ...
    const handleUpdateFournisseur = async (updated: Fournisseur) => {
        ...
        try {
            const { id, ...properties } = updated;
            await dbUpdateDoc<Fournisseur>('fournisseurs', id, properties);
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md">
                        ...
                        >
                            ...
                            {activeAlarmTask.notes && (
                                <div className="mt-3 p-3 bg-slate-50 rounded-xl border border-gray-200 text-left w-full">
