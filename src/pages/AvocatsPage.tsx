...
const AvocatsPage: FC<AvocatsPageProps> = ({ avocats, tasks = [], onAddAvocat, onSendEmail, correspondances = [] }) => {
    ...
    const getServiceStatusClass = (status: string) => {
        switch (status) {
            ...
            case 'Mise en disponibilité': return 'bg-blue-100 text-blue-800';
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-700 font-medium p-3 bg-slate-50 border border-slate-100 rounded-xl space-y-2">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-700 font-semibold leading-relaxed flex items-center gap-1.5"><CalendarIcon className="w-3.5 h-3.5 text-indigo-600" /> 1er Serment : {selectedAvocat.firstOathDate || 'N/A'}</p>
                                        {selectedAvocat.secondOathDate && (
                                            <p className="text-xs text-gray-700 font-semibold leading-relaxed mt-1 flex items-center gap-1.5"><CalendarIcon className="w-3.5 h-3.5 text-indigo-600" /> 2nd Serment : {selectedAvocat.secondOathDate}</p>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        {selectedAvocat.secondaryBar ? (
                                            <p className="text-xs text-slate-800 font-semibold leading-relaxed mt-1.5 flex items-center gap-1.5">
                                                ...
                                                <span className="bg-slate-100 border border-slate-200 text-slate-700 font-bold px-1.5 py-0.5 rounded text-2xs">{selectedAvocat.secondaryBar}</span>
                                            </p>
                                        ) : (
                                            ...
                                            <div key={letter.id} className="p-3 bg-indigo-50/20 border border-indigo-100/50 rounded-xl flex items-center justify-between hover:bg-indigo-50/50 transition duration-150">
                                                <div className="min-w-0 flex-1">
                                                    ...
                                                    <span className="text-[10px] text-slate-500 font-medium">
                                                        Destinataire : <strong className="font-semibold text-slate-600">{letter.recipientName || letter.destinataire}</strong>
