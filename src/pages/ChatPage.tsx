            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 w-fit">
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg text-xs md:text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'direct' ? 'bg-white shadow-sm text-indigo-700' : 'text-gray-500 hover:text-gray-800'}`}
                    >
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg text-xs md:text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'forum' ? 'bg-white shadow-sm text-indigo-700' : 'text-gray-500 hover:text-gray-800'}`}
                    >
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                {filteredChatUsers.map(person => (
                                     <li key={person.name}>
                                        <button
                                            ...
                                            className={`w-full text-left p-4 hover:bg-slate-100/55 flex items-center justify-between transition-all duration-200 ${selectedContact?.name === person.name ? 'bg-indigo-50/70 border-l-4 border-indigo-600' : 'border-l-4 border-transparent'}`}
                                        >
                                    ...
                                    filteredForumTopics.map(topic => (
                                        <li key={topic.id}>
                                            <button
                                                ...
                                                className={`w-full text-left p-4 hover:bg-slate-100/55 flex flex-col justify-between transition-all duration-200 border-l-4 ${selectedTopicId === topic.id && !isCreatingTopic ? 'bg-indigo-50/75 border-indigo-600' : 'border-l-4 border-transparent'}`}
                                            >
                            ...
                            {isCreatingTopic ? (
                                ...
                                <div className="flex flex-col h-full bg-white select-none">
                                    ...
                                    <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-slate-50/20">
                                        ...
                                        <div className="bg-gradient-to-br from-slate-50 to-white border border-slate-200 rounded-2xl p-4.5 shadow-3xs relative">
                                            ...
                                            <p className="text-xs text-slate-700 font-medium leading-relaxed whitespace-pre-line">{activeForumTopic.content}</p>
                                        </div>
                                ...
                                <div className="flex-1 flex items-center justify-center text-gray-500">
                                   <div className="text-center p-8 max-w-sm">
                                        ...
                                        <h3 className="text-sm font-bold text-slate-700 mt-4">Sélectionnez un sujet</h3>
                                        <p className="text-xs text-gray-400 mt-1.5 leading-relaxed">
