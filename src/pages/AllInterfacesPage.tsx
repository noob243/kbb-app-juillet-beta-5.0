...
const AllInterfacesPage: FC<AllInterfacesPageProps> = (props) => {
    const dummySendEmail = props.onSendEmail || (() => {});
    return (
        <div className="space-y-12">
            <div>
                <h2 className="text-2xl font-bold text-gray-700 mb-4 pb-2 border-b-2 border-indigo-500">Tableau de Bord</h2>
                <DashboardPage clients={props.clients} cases={props.cases} events={props.events} tasks={props.tasks} invoices={props.invoices} avocats={props.avocats} />
            </div>
             <div>
                <h2 className="text-2xl font-bold text-gray-700 mb-4 pb-2 border-b-2 border-indigo-500">Clients</h2>
                <ClientsPage clients={props.clients} onAddClient={props.onAddClient} onExport={props.onExportClients} onSendEmail={dummySendEmail} cases={props.cases} invoices={props.invoices} tasks={props.tasks} />
            </div>
            <div>
                <h2 className="text-2xl font-bold text-gray-700 mb-4 pb-2 border-b-2 border-indigo-500">Dossiers</h2>
                <CasesPage cases={props.cases} clients={props.clients} onAddCase={props.onAddCase} onExport={props.onExportCases} avocats={props.avocats} invoices={props.invoices} tasks={props.tasks} onSendEmail={dummySendEmail} />
            </div>
            <div>
                <h2 className="text-2xl font-bold text-gray-700 mb-4 pb-2 border-b-2 border-indigo-500">Événements</h2>
                <EventsPage events={props.events} onAddEvent={props.onAddEvent} avocats={props.avocats} />
            </div>
            <div>
                <h2 className="text-2xl font-bold text-gray-700 mb-4 pb-2 border-b-2 border-indigo-500">Agenda</h2>
                <AgendaPage tasks={props.tasks} cases={props.cases} lawyers={props.lawyerNames} avocats={props.avocats} onAddTask={props.onAddTask} events={props.events} />
            </div>
             <div className="h-[700px]">
                <h2 className="text-2xl font-bold text-gray-700 mb-4 pb-2 border-b-2 border-indigo-500">Chat</h2>
                <ChatPage avocats={props.avocats} personnels={[]} currentUserInfo={null} presences={{}} />
            </div>
             <div>
                <h2 className="text-2xl font-bold text-gray-700 mb-4 pb-2 border-b-2 border-indigo-500">Facturation</h2>
                <BillingPage invoices={props.invoices} cases={props.cases} onAddInvoice={props.onAddInvoice} clients={props.clients} onSendEmail={dummySendEmail} />
            </div>
             <div>
                <h2 className="text-2xl font-bold text-gray-700 mb-4 pb-2 border-b-2 border-indigo-500">Avocats</h2>
                <AvocatsPage avocats={props.avocats} onAddAvocat={props.onAddAvocat} onSendEmail={dummySendEmail} tasks={props.tasks} />
            </div>
        </div>
    );
};
