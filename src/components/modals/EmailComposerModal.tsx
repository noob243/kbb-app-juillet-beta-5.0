...
const EmailComposerModal: FC<EmailComposerModalProps> = ({
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    // Save sent email record as correspondance in Firestore
    try {
      await dbCreateDoc('correspondances', `corr_${Date.now()}`, {
        id: `corr_${Date.now()}`,
        type: 'Sortant',
        expediteur: 'Cabinet KBB',
        destinataire: recipientName || to,
        objet: subject,
        date: new Date().toISOString().split('T')[0],
        status: 'Envoyé',
        contenu: body,
        clientName: recipientName || to,
        canal: 'Email'
      });
    } catch (e) {
      console.warn("Failed to persist sent email to Firestore correspondances:", e);
    }
